...
async function requireLogin(redirectTo = '../login.html') {
  ...
  // Max-2-device check — if this device got evicted (3rd device logged in elsewhere), sign out.
  const deviceId = getDeviceId();
  const { data: valid, error } = await supabaseClient.rpc('check_device_session', { p_device_id: deviceId });
  if (error || !valid) {
    await supabaseClient.auth.signOut();
    alert('You were logged out because this account was used on another device. Only 2 devices can be active at a time.');
    window.location.href = redirectTo;
    return null;
  }
  return session;
...
// Persistent per-browser device id, used for the 2-device login limit.
...
async function isMaintenanceOn(profile) {
  ...
  const { data } = await supabaseClient.from('app_settings_public').select('maintenance_mode').eq('id', 1).single();
...
// Any other URL (GitHub raw, direct .pdf link, etc.) is returned unchanged.
function toPdfPreviewUrl(url){
  const m = url.match(/drive\.google\.com\/file\/d\/([^/]+)/);
  if (m) return `https://drive.google.com/file/d/${m[1]}/preview`;
  return url;
}
// Converts a normal Google Drive share link into a direct-download URL.
function toPdfDownloadUrl(url){
  const m = url.match(/drive\.google\.com\/file\/d\/([^/]+)/);
